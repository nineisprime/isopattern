...
range = c()
for (i in 1:(2*floor(n^(1/2))+1) ){
    range[i] = -floor(n^(1/2)) + i - 1
}

nr = length(range)

slits = matrix(0, nr, d)
slits[, 1] = floor(n/5) + range
slits[, 2] = floor(n/5) + range
slits[, 3] = floor(4*n/5) + range
...
for (j in 1:d){
    ...
    P_ls[[j]] = P
    allX = cbind(allX, P %*% isomatX[, slits[, j]])
}
...
deltastar = matrix(0, nr, d)
deltastar[ceiling(nr/2), 1] = 1
deltastar[ceiling(nr/2), 2] = 1
deltastar[ceiling(nr/2), 3] = 1
...
epsilon = rep(0, n)
y = allX %*% matrix(deltastar, d*nr, 1)
...
deltahat = matrix(as.vector(glmfit$beta), nr, d)





f1hat = isomatX[, slits[, 1]] %*% deltahat[, 1]
f2hat = isomatX[, slits[, 2]] %*% deltahat[, 2]
f3hat = isomatX[, slits[, 3]] %*% deltahat[, 3]

f1 = isomatX[, slits[, 1]] %*% deltastar[, 1]
f2 = isomatX[, slits[, 2]] %*% deltastar[, 2]
f3 = isomatX[, slits[, 3]] %*% deltastar[, 3]
