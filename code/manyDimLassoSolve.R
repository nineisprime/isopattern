...
Sigma = -0.9 * matrix(1, d, d)
Sigma[1:d1, 1:d1] = .9
Sigma[(d1+1):d, (d1+1):d] = .9
diag(Sigma) = 1
...
for (i in 1:d) {
    if (i <= d1)
        next 
    y1 = y1 - t(isomatX) %*% t(P_ls[[1]]) %*% P_ls[[i]] %*%
             (isomatX %*% (deltahat[, i]))
    y2 = y2 - t(isomatX) %*% t(P_ls[[1]]) %*% P_ls[[i]] %*%
             (isomatX %*% (deltastar[,i]))
}
plot(1:(n-1), y1, type="l", col="blue")
lines(1:(n-1), y2, col="red")
